...
class AddressController {
  ...
  async createAddress(req, res, next) {
    try {
      const { lat, lng, fullName, phone, myself,address, province, city } = req.body;
  ...
  async editAddress(req, res, next) {
    try {
      const { addressId, lat, lng, fullName, phone, myself,address, province, city } = req.body;
      const { mobile, userId } = req.user;
      let data;
  ...
  async removeAddress(req, res, next) {
    try {
      ...
      if (!address)
        throw createHttpError.NotFound(AddressMessages.NotFoundAddress);
